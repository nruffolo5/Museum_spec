...
# Read in the data from the museum taken outside in direct sunlight
...
plot(outside_small, col = "white", ylim = c(0, max(outside_small[, -1])), main = "Outside: Small Birds Reflectance")
...
outside_big <- read_xlsx("Outside_big.xlsx", skip = 5)

# Removes sd columns
outside_big <- outside_big[,seq(1,175,1)]

# Print column names to identify relevant columns
print(colnames(outside_big))

# Identify columns matching the wavelength pattern
wavelength_cols <- grep("^\\d+\\.\\.\\.", colnames(outside_big), value = TRUE)

# Select relevant columns dynamically
outside_big <- outside_big %>% select(name...1, all_of(wavelength_cols))
...
# Create an identifier for each specimen and separate metadata
outside_big <- outside_big %>%
  separate(name...1, into = c("id", "img.number", "date", "time"), sep = "_", extra = "merge", fill = "right")

# Check for duplicate IDs
if (sum(duplicated(outside_big$id)) > 0) {
  message("Warning: Duplicate IDs detected. Making them unique.")
  outside_big <- outside_big %>%
    mutate(id = make.unique(id))
}
...
# Remove existing row names before assigning new ones
rownames(outside_big) <- NULL  

# Set 'id' as row names
outside_big <- outside_big %>% column_to_rownames(var = "id")

# Remove unnecessary columns
outside_big <- outside_big %>% select(-c("img.number", "date", "time"))

# Transpose (flip) the data frame
outside_big <- t(outside_big)

# Turning outside_big into a base R dataframe
outside_big <- data.frame(outside_big)

# Checking that it has the correct structure
str(outside_big)

# Converting data into a percentage from a proportion
outside_big <- outside_big[,-1] * 100

# Add a column for the wavelengths extracted from perClass Mira
outside_big <- cbind(seq(350, 1002, 4), outside_big)
colnames(outside_big)[1] <- "wl"

# Convert the dataset into an 'rspec' object, which is required for 'pavo'
outside_big <- as.rspec(outside_big, interp = TRUE, lim = c(300,700))

# Convert negative reflectance values to 0
outside_big <- procspec(outside_big, fixneg = "zero")

# Plot the reflectance data
plot(outside_big)

# First plot: General reflectance
plot(outside_big, col = "white", ylim = c(0, max(outside_big[, -1])), main = "Outside: Large Birds Reflectance")
for (i in 2:length(colnames(outside_big))) {
  reflectance <- outside_big[, i]
  lines(x = 350:700, y = reflectance[-c(1:50)], col = spec2rgb(as.rspec(data.frame(wl = outside_big$wl, reflectance = outside_big[, i]))), lwd = 2)
}

# Second plot: Limited range
plot(outside_big, col = "white", ylim = c(0, 100), main = "Outside: Large Birds Reflectance", xlim = c(400, 700))
for (i in 2:length(colnames(outside_big))) {
  reflectance <- outside_big[, i]
  lines(x = 350:700, y = reflectance[-c(1:50)], col = spec2rgb(as.rspec(data.frame(wl = outside_big$wl, reflectance = outside_big[, i]))), lwd = 2)
}
...
# Third plot: Adjusted transparency
plot(outside_big, col = "white", ylim = c(0, 100), main = "Outside Large Birds Reflectance", xlim = c(380, 700))
for (i in 2:length(colnames(outside_big))) {
  reflectance <- outside_big[, i]
  lines(x = 350:700, y = reflectance[-c(1:50)], col = adjustcolor(spec2rgb(as.rspec(data.frame(wl= outside_big$wl, reflectance = outside_big[, i]))), 0.5), lwd = 3)
}
